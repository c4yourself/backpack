...
function TestColor:test_from_table()
	luaunit.assertEquals(
		color.Color.from_table({}):to_table(),
		{0, 0, 0, 255})
	luaunit.assertEquals(
		color.Color.from_table({255, 255, 255}):to_table(),
		{255, 255, 255, 255})
	luaunit.assertEquals(
		color.Color.from_table({255, 255, 255, 255}):to_table(),
		{255, 255, 255, 255})
	luaunit.assertEquals(
		color.Color.from_table({r=255, g=255, b=255, a=255}):to_table(),
		{255, 255, 255, 255})
	luaunit.assertEquals(
		color.Color.from_table({red=255, green=255, blue=255, alpha=255}):to_table(),
		{255, 255, 255, 255})

	luaunit.assertError(color.Color.from_table, 1)
	luaunit.assertError(color.Color.from_table, {grue=255})
end

function TestColor:test_to_html()
	luaunit.assertEquals(color.Color(255, 255, 255, 255):to_html(), "#ffffff")
	luaunit.assertEquals(color.Color(255, 255, 255, 0):to_html(), "#ffffff00")
end
